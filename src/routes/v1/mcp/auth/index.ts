...
export class ProxyOAuthServerProvider {
   ...
   */
  private async introspectToken(token: string): Promise<{ valid: boolean; payload?: any }> {
    const response = await fetch(this.authorizationServerMetadata.introspection_endpoint, {
      method: "POST",
      headers: {
   ...
   */
  private async validateWithUserinfo(token: string): Promise<{ valid: boolean; payload?: any }> {
    const response = await fetch(this.authorizationServerMetadata.userinfo_endpoint, {
      headers: {
   ...
   */
  async authenticateRequest(c: Context): Promise<{ authenticated: boolean; user?: any }> {
  ...
  getResourceMetadata() {
    return {
      ...
      scopes_supported: this.config.scope ? [this.config.scope] : ["read", "write"],
